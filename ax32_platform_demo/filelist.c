...
static int filelistScanStart(int handle)
{
	...
	fd = open(filelistNode[handle].ordername,FA_WRITE | FA_READ);
//	deg_Printf("cleate file: %s.%d\n",filelistNode[handle].ordername,fd);
	if(fd<0)
	{
		//deg_Printf("open list file: %s err!!!\n",filelistNode[handle].ordername);
		fd = open(filelistNode[handle].ordername,FA_WRITE | FA_READ|FA_CREATE_NEW);
		f_chmod((char const *)filelistNode[handle].ordername,6,0x06);
		if(fd<0)
		{
			//deg_Printf("cleate list file: %s err!!!\n",filelistNode[handle].ordername);
			return -1;
		}
		filelistNode[handle].count = 0;
		filelistNode[handle].aoffset=0;
...
static int filelistFlush(int handle)
{
	...
	if(filelistHandle<0)
		filelistHandle = open(filelistNode[handle].ordername,FA_WRITE | FA_READ);
	else		
