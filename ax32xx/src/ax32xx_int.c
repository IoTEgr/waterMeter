...
__except  void delay(uint32_t cnt)
{
    while(cnt--) asm("l.nop");
}
...
__except void ax32xx_WriteByte(u8 byte)
{
	volatile int timeout = 0x8000;
	RTCDATA = byte;
	while(RTCCON & (1<<3))
...
__except void ax32xx_DataWrite(u8 cmd,u8 *buffer,u8 len)
{
	int i;
	
	RTCCON |= (1<<8);        //CE ENABLE 

	ax32xx_WriteByte(cmd);	
    for(i=0;i<len;i++)
    {
		ax32xx_WriteByte(buffer[i]);
    }
	RTCCON &= ~(1<<8);       //CE DISABLE 	
}

__except void ax32xx_WKOEnable2(u8 en)
{
	u8 temp;
	RTCCON |= (1<<6);
    if(en)
	...
	ax32xx_DataWrite(IRTCON4_WR_CMD,&temp,1);
}
