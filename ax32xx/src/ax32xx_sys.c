...
//-----system power
    ...
    R_SYS_LDO |= (0<<21)|//VDDSD en15
    			 //(0<<6)|//sensor ldo use select, 0: use vcc3.3V , 1 use VDD_RTC
				 //(0x0f<<16)|//bandgap voltage select (3<<12)
				 (1<<15);//adkey bandgap enable11

	ax32xx_sysLDOSet(SYS_LDO_V1_2,SYS_VOL_V1_25,1);
...
//----INT initial
	...
	uart_Printf("(u32)&_sdram_remian_addr=0x%x\n",(u32)&_sdram_remian_addr);
	uart_Printf("(u32)&__sdram_remain_size=0x%x\n",(u32)&__sdram_remain_size);

    if(saddr)
		*saddr = (u32)&_sdram_remian_addr;
	if(eaddr)
		*eaddr = ((u32)&_sdram_remian_addr)+((u32)&__sdram_remain_size);
