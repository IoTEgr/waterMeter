...
u8 pack_total, pack_len, pack_total_len;
...
int imageEncodeStart(FHANDLE fileHanle, INT16U image_width, INT16U image_height, INT8U image_q, INT8U timestramp)
{
	...
	hal_mjpegPhotoStart(image_width, image_height, image_q, 0);
...
{
	INT32S res;
	INT32U addr, size, timeout;
	void *buff;
	s32 sync, sync_next;
	INT16U image_width = 1280, image_height = 720;
	u32 i;
	...
	hal_mjpegPhotoStart(image_width, image_height, JPEG_Q_28, 0); // JPEG_Q_81
	...
	while (1)
	{
		...
		if (buff)
		{
			...
			if (uart_buff == NULL)
			{
				uart_buff = hal_sysMemMalloc(size, 32);
			}

			if (uart_buff)
			{
				memcpy(uart_buff, addr, size);
				uart_buff_len = size;

				pack_total = ((uart_buff_len + 0x3ff) & (~0x3ff)) / 1024; // 每一包最大1024 ，总包数
				deg_Printf("---pack_total=%d--.\n", pack_total);
			}
			else
			{
				deg_Printf("uart buff : memory malloc fail\n");
			}
	...
	{
		fHandle = open("1.jpg",FA_CREATE_NEW | FA_WRITE | FA_READ);
		if(!(fHandle<0))
		{
			write(fHandle, uart_buff ,size);
			close(fHandle);
		}
	}
#endif
