...
void videoRecordImageWatermark(INT16U width,INT16U height,INT8U en)
{
    ...
    if(en)
    {
		deg_Printf("watermark :mediaVideoCtrl.tinfo=%d\n",mediaVideoCtrl.tinfo);
...
//note flag no implement !!!!,only is offset form start
int video_avi_seek(int fd,int offset,int flag)
{
	int cnt;

	cnt = lseek(fd,offset,flag);
...
//-----------initial timestramp------------------------
	...
	{
		videor_print("video record : mediaVideoCtrl.tinfo=%d\n",mediaVideoCtrl.tinfo);
	...
	//	mediaVideoCtrl.space-=size>>10;
		...
		if(mediaVideoCtrl.arg.ftype == AVI_TYPE_OPENDML)
		{
			if(8==size)
			{
				...
				temp[2]=0x4B4E554A;
				temp[3]=512-16;
				res = write(mediaVideoCtrl.aviFd,(void *)addr,512);
				if(512==res)
				{
					res=8;
					aviEncodeAddofs(mediaVideoCtrl.aviFd,512-8);
				}
			}
			else
			{
		...
		if(videoRecordUpdateAviIdx1(&mediaVideoCtrl,AVI_FRAME_DC_VIDEO,insert,!icnt)<0)
		{
			sysLog("error no 2");
			res = 2;
			hal_mjpegRawDataSet(buff_vids);	
...
void videoRecordSetResolution(u32 res)
{
	mediaVideoCtrl.arg.resolution.width=res>>16;
	mediaVideoCtrl.arg.resolution.height=res&0xffff;
}
