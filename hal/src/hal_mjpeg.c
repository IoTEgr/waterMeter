		...
		else
		{
			...
			if(flag & (BIT (MJPEG_IRQ_FRAMEEND) | BIT(MJPEG_IRQ_OUTPAUSE)))
			{
				...
				if(!mjpegEncCtrl.drop){
					...
					{
						//deg_Printf("MJPEG PHOTO finish:%d\n",len);
...
//	MJPEG_ITEM_SIZE = 32768;
	...
	#else
	    size = hal_sysMemRemain()-8*1024-250*1024;
		if(0 == task)	// photo not pcm data
		{
			size -= HAL_CFG_PCM_BUFFER_SIZE*HAL_CFG_PCM_BUFFER_NUM;
		}
	#endif	
...
{  
	...
	{
		//win_w=(configGet(CONFIG_ID_MJPEG_W)+ 0x1f) & (~0x1f);
		//win_h=configGet(CONFIG_ID_MJPEG_H) & ~1;

		win_w=configGet(CONFIG_ID_MJPEG_W);
		win_h=configGet(CONFIG_ID_MJPEG_H);
		...
		crop_w = win_w;
		crop_h = win_h;
		sx = configGet(CONFIG_ID_CENTER_X);
		ex = configGet(CONFIG_ID_CENTER_X)+win_w;
		sy = configGet(CONFIG_ID_CENTER_Y);
		ey = configGet(CONFIG_ID_CENTER_Y)+win_h;
		...
		ax32xx_mjpegEncodeQuilitySet(quality);
		ax32xx_mjpegEncodeInfoSet(0);
		videoRecordImageWatermark(win_w,win_h,0);
...
//=====crop=====
		...
		if(640==csi_w)
		{
			...
			uart_Printf("sx=%d,sy=%d,ex=%d,ey=%d\n",sx,sy,ex,ey);
			ax32xx_csiScaler(csi_w,csi_h, crop_w, crop_h,sx,sy,ex,ey);
			ax32xx_mjpegEncodeSizeSet(crop_w,crop_h, win_w,win_h);
...
* Return         : None
*******************************************************************************/
static int hal_mjpegHeaderParse (u8 * p_jpeg_header)
{
#if 0
	u8 *seg, b;
	u16 marker;
